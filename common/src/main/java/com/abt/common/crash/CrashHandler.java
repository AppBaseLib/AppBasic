...
import com.abt.common.app.Constant;
...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		Log.e(TAG, "uncaughtException, threadName:" + thread.getName() + ", threadId:" + thread.getId());
		handleException(ex);
		try {
			Thread.sleep(2500);
		...
		Intent i = new Intent();
		i.setAction(Constant.QIUT_APPLICATION_ACTION);
	...
	private String getDataTime() {
		...
		ret = String.format("%04d", year) + String.format("%02d", month) + String.format("%02d", day)
	...
	private boolean handleException(Throwable ex) {
		...
		// //to.add("zhanhong.wang@tcl.com");
		// //to.add("fuyan.zhang@tcl.com");
		// //to.add("xingqiang.man@tcl.com");
		// //to.add("hui.zhou.sz@jrdcom.com");
		// try {
		// EmailUtils.sendEmail(to, "TCL净化器APP异常", msg);
	...
	public static void writeFileData(Context ctx, String fileName, String message) {
		try {
